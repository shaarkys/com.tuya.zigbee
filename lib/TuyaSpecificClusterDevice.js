...
class TuyaSpecificClusterDevice extends ZigBeeDevice {
    ...
    async writeData32(dp, value, { throwOnError = false } = {}) {
        ...
        if (normalizedValue < -0x80000000 || normalizedValue > 0xFFFFFFFF) {
            throw new RangeError('DATA32 value must be an integer between -2147483648 and 4294967295');
        }
        const data = Buffer.alloc(4);
        if (normalizedValue < 0) {
            data.writeInt32BE(normalizedValue, 0);
        } else {
            data.writeUInt32BE(normalizedValue, 0);
        }
        this.log('[WRITE DATA32] DP:', dp, 'Value:', value, 'Normalized:', normalizedValue, 'Datatype: 2, Length: 4, Data:', data);
        try {
            return await this._sendTuyaDatapoint({
                dp,
                datatype: 2,
